...
func moveCrates(crates *[][]string, scanner *bufio.Scanner) {
	for scanner.Scan() {
        ...
        if len((*crates)[sn[1]-1]) > sn[0] {
            (*crates)[sn[2]-1] = append((*crates)[sn[2]-1], (*crates)[sn[1]-1][len((*crates)[sn[1]-1])-sn[0]:]...)
            (*crates)[sn[1]-1] = (*crates)[sn[1]-1][:len((*crates)[sn[1]-1])-sn[0]]
        } else {
...
func main() {
    ...
    for i := 0; i < len(crates); i++ {
		fmt.Println(crates[i][len(crates[i])-1])
	}
}
